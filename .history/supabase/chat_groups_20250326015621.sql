...
-- Simplified policies for chat_messages table
-- First, drop all existing policies
DROP POLICY IF EXISTS "Anyone can view messages" ON public.chat_messages;
DROP POLICY IF EXISTS "Open groups allow anyone to post messages" ON public.chat_messages;
DROP POLICY IF EXISTS "Broadcast groups allow only admins to post messages" ON public.chat_messages;
DROP POLICY IF EXISTS "Users can update their own messages" ON public.chat_messages;
DROP POLICY IF EXISTS "Users can delete their own messages" ON public.chat_messages;

-- Then create simplified policies
CREATE POLICY "Anyone can view messages" 
  ON public.chat_messages 
  FOR SELECT
  USING (true);

-- Allow any inserts to chat_messages table (we'll handle permissions in the application code)
CREATE POLICY "Allow all message inserts" 
  ON public.chat_messages 
  FOR INSERT
  WITH CHECK (true);
...
-- Modified seed data for broadcast groups with admin IDs (will be assigned later)
-- They will be empty initially but should be updated with actual user IDs when users are created
INSERT INTO public.chat_groups (name, type, description, is_active, admin_id) VALUES
  (
    'Ankündigungen Test e.V.',
    'broadcast',
    'Offizielle Ankündigungen des Test e.V.',
    true,
    NULL -- To be assigned later through an UPDATE statement
  ),
  (
...
-- Helper function to set admin ID for broadcast groups
CREATE OR REPLACE FUNCTION public.assign_admin_to_broadcast_groups()
RETURNS void AS $$
DECLARE
    first_user_id UUID;
BEGIN
...
-- Execute the function to assign admin
SELECT public.assign_admin_to_broadcast_groups();
