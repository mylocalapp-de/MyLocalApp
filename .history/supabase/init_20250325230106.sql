...
-- Allow the system to manage all profiles (for trigger function)
...
-- Allow the system to manage all preferences (for trigger function)
...
-- Helper function to insert user preferences (bypassing RLS)
CREATE OR REPLACE FUNCTION public.insert_user_preferences(p_user_id UUID, p_preferences TEXT[])
RETURNS VOID AS $$
BEGIN
  -- First delete any existing preferences to avoid duplicates
  ...
  SELECT 
    p_user_id,
    preference
  FROM unnest(p_preferences) AS preference;
  
  RAISE NOTICE 'Inserted % preferences for user %', array_length(p_preferences, 1), p_user_id;
END;
$$ LANGUAGE plpgsql SECURITY DEFINER SET search_path = public;
...
GRANT EXECUTE ON FUNCTION public.insert_user_preferences(UUID, TEXT[]) TO authenticated;
