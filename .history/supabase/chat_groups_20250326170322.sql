...
CREATE TABLE public.chat_groups (
  ...
  description TEXT,
  tags TEXT[] DEFAULT '{}', -- Add tags as an array of strings
...
INSERT INTO public.chat_groups (id, name, type, description, tags, is_active) VALUES
  (
...
BEGIN
  IF NOT EXISTS (SELECT 1 FROM public.chat_groups WHERE name = 'Offene Gruppe Straßenbau') THEN
    INSERT INTO public.chat_groups (name, type, description, tags, is_active) 
    VALUES (
      'Offene Gruppe Straßenbau',
      'open_group',
      'Diskussion über lokale Straßenbauarbeiten',
      ARRAY['Verkehr', 'Infrastruktur', 'Bauprojekte'],
      true
    );
  END IF;

  -- Add tags to existing Straßenbau group if it exists but has no tags
  UPDATE public.chat_groups
  SET tags = ARRAY['Verkehr', 'Infrastruktur', 'Bauprojekte']
  WHERE name = 'Offene Gruppe Straßenbau' AND (tags IS NULL OR tags = '{}');

  IF NOT EXISTS (SELECT 1 FROM public.chat_groups WHERE name = 'Offene Labergruppe') THEN
    INSERT INTO public.chat_groups (name, type, description, tags, is_active) 
    VALUES (
      'Offene Labergruppe',
      'open_group',
      'Allgemeine Diskussionen',
      ARRAY['Allgemein', 'Sonstiges'],
      true
    );
  END IF;

  -- Add tags to existing Labergruppe if it exists but has no tags
  UPDATE public.chat_groups
  SET tags = ARRAY['Allgemein', 'Sonstiges']
  WHERE name = 'Offene Labergruppe' AND (tags IS NULL OR tags = '{}');
END $$;
...
BEGIN
  ...
  -- Add tags to existing Test e.V. if it exists but has no tags
  UPDATE public.chat_groups
  SET tags = ARRAY['Vereine', 'Sport']
  WHERE name = 'Ankündigungen Test e.V.' AND (tags IS NULL OR tags = '{}');
  ...
  IF NOT EXISTS (SELECT 1 FROM public.chat_groups WHERE name = 'Kunsthaus Nachrichten') THEN
    ...
    VALUES (
      ...
      ARRAY['Kultur', 'Veranstaltungen'],
      true
    );
  END IF;
  
  -- Add tags to existing Kunsthaus if it exists but has no tags
  UPDATE public.chat_groups
  SET tags = ARRAY['Kultur', 'Veranstaltungen']
  WHERE name = 'Kunsthaus Nachrichten' AND (tags IS NULL OR tags = '{}');
END $$;
