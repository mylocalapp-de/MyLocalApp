...
BEGIN
  ...
  FROM public.app_users
  WHERE app_users.email = verify_password.email;
...
EXCEPTION WHEN undefined_function THEN
  ...
  FROM public.app_users
  WHERE 
    app_users.email = verify_password.email AND
...
BEGIN
  -- Handle NULL inputs
  IF p_stored_hash IS NULL OR p_password IS NULL THEN
    RAISE LOG 'verify_custom_password: NULL inputs provided';
    RETURN FALSE;
  END IF;

  -- Log for debugging (remove in production)
  RAISE LOG 'verify_custom_password: Comparing password for hash';
  
  -- For bcrypt, we can use the stored hash as the salt
  BEGIN
    -- Try using crypt (for bcrypt)
    RETURN p_stored_hash = crypt(p_password, p_stored_hash);
  EXCEPTION 
    WHEN undefined_function THEN
      -- Fallback to MD5 if crypt isn't available
      RAISE WARNING 'Falling back to MD5 comparison';
      RETURN p_stored_hash = md5(p_password);
    WHEN OTHERS THEN
      -- Log any other errors but don't expose details to caller
      RAISE WARNING 'Error in verify_custom_password: %', SQLERRM;
      RETURN FALSE;
  END;
EXCEPTION WHEN OTHERS THEN
  -- Catch-all for any unexpected errors
  RAISE WARNING 'Unexpected error in verify_custom_password: %', SQLERRM;
  RETURN FALSE;
END;
$$ LANGUAGE plpgsql SECURITY DEFINER;
