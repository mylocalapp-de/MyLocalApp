...
BEGIN
  -- Debug logs
  RAISE LOG 'Trigger firing for new user: %', new.id;
  ...
  -- Try to get preferences from different locations in metadata
  BEGIN
    -- Check multiple possible locations for preferences data
    IF new.raw_user_meta_data IS NOT NULL THEN
      ...
      ELSIF new.user_metadata IS NOT NULL AND new.user_metadata->>'preferences' IS NOT NULL THEN
    ...
    IF preferences_json IS NOT NULL THEN
      ...
      IF jsonb_typeof(preferences_json) = 'array' THEN
        ...
        RAISE LOG 'Successfully inserted array preferences for user: %', new.id;
      ELSIF jsonb_typeof(preferences_json) = 'object' THEN
        ...
        RAISE LOG 'Successfully inserted object preferences for user: %', new.id;
      ELSE
        RAISE LOG 'Unrecognized preferences format: %', jsonb_typeof(preferences_json);
      END IF;
    ELSE
      RAISE LOG 'No preferences data found for user: %', new.id;
    END IF;
  EXCEPTION WHEN OTHERS THEN
...
VALUES 
  ('00000000-0000-0000-0000-000000000000', 'test@example.com', null)
