...
-- Create policies for event_reactions table
CREATE POLICY "Anyone can view event reactions" 
  ON public.event_reactions 
  FOR SELECT
  USING (true);

CREATE POLICY "Authenticated users can create event reactions" 
  ON public.event_reactions 
  FOR INSERT
  WITH CHECK (auth.uid() IS NOT NULL);

CREATE POLICY "Users can update their own event reactions" 
  ON public.event_reactions 
  FOR UPDATE
  USING (user_id = auth.uid())
  WITH CHECK (user_id = auth.uid());
...
CREATE POLICY "Authenticated users can create event attendees" 
  ON public.event_attendees 
  FOR INSERT
