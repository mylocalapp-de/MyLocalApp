      ...
      if (userId) {
        try {
          ...
          if (rpcError) {
            ...
            // Instead of using OTP, directly create a session manually
            // This approach is recommended for React Native according to Supabase community
            if (userEmail && userId) {
              console.log('Creating session manually for new user:', userEmail);
              try {
                // Generate a temporary session token and set it
                ...
                if (error) {
                  ...
                  // If we can't generate a link, try to get user info at least
                  const { data: userData } = await supabase.auth.admin.getUserById(userId);
                  if (userData) {
                    console.log('Got user data:', userData.user.id);
                  }
                } else if (data) {
                  // Try to set the session with the tokens from the generated link
              ...
              // If all session creation approaches fail, simply notify the user and navigate back
              Alert.alert(
                'Konto erstellt',
                'Dein Konto wurde erstellt. Du kannst dich jetzt anmelden mit deiner E-Mail-Adresse.',
                [{ text: 'OK' }]
              );
              console.log('Navigating back to Welcome screen after account creation');
