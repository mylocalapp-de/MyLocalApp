  ...
  const handleComplete = async () => {
    ...
    try {
      ...
      if (userId) {
        try {
          ...
          if (rpcError) {
            ...
            // Instead of navigating directly, refresh the auth session to update the user state
            // This will trigger the conditional rendering in the AppNavigator
            try {
              console.log('Refreshing session to trigger navigation change');
      ...
      if (error) {
        ...
        // Success - refresh the session instead of trying to navigate directly
        try {
          console.log('Refreshing session to trigger navigation change');
          const { data, error } = await supabase.auth.refreshSession();
          if (error) {
