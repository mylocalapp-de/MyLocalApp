...
const ProfileScreen = () => {
  ...
  // Use auth context
  const { user, preferences, signOut, upgradeToFullAccount, resetOnboarding } = useAuth();
  ...
  useEffect(() => {
    console.log('ProfileScreen - Initial user state:', user ? `User ID: ${user.id}` : 'No user');
  ...
  const handleCreateAccount = async () => {
    ...
    try {
      ...
      if (result.success) {
        console.log('Account created successfully with ID:', result.user?.id);
        
        // Verify data was properly stored in Supabase tables
        if (result.user?.id) {
          console.log('Verifying database tables were updated...');
          const verificationResult = await verifyProfileCreated(result.user.id);
