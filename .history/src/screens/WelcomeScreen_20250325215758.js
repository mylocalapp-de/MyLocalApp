...
const WelcomeScreen = ({ navigation }) => {
  ...
  const { signIn, createLocalUser } = useAuth();
  ...
  const handleSignUp = async () => {
    ...
    try {
      ...
      // Create local user instead of Supabase account
      const { success, error, user } = await createLocalUser(email);
      
      if (error || !success) {
        console.error('Error creating local user:', error);
        Alert.alert('Fehler bei der Registrierung', error?.message || 'Ein Fehler ist aufgetreten');
      } else {
        console.log('Local user created, navigating to onboarding');
            ...
            {!showLoginForm && !showSignupForm ? (
              ...
              <View style={styles.formContainer}>
                ...
                <Text style={styles.formMessage}>
                  Du kannst direkt loslegen! Ein Passwort kannst du später in deinem Profil festlegen.
                </Text>
