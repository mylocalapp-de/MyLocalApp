...
const CalendarScreen = () => {
  ...
  // German month names for use in formatMonth function
  const monthNames = {
    1: 'Januar',
    2: 'Februar',
    3: 'März',
  ...
  // Day names in German (starting with Sunday)
  ...
  // German month formatting function
    ...
    <SafeAreaView style={styles.container}>
      ...
      <View style={styles.calendarContainer}>
        <Calendar
          ...
          // Custom day names to use German abbreviations
          dayComponent={({ date, state, marking }) => {
            // For the day headers (Sun, Mon, etc)
            if (!date) {
              return <Text style={{ fontSize: isSmallScreen ? 10 : 12, color: '#666' }}>{dayNames[date?.day || 0]}</Text>;
            }
            ...
            return (
              <View style={{
                ...
                alignItems: 'center',
                backgroundColor: isSelected ? (marking?.startingDay || marking?.endingDay ? '#4285F4' : '#80b3ff') : 'transparent',
                borderRadius: isSelected ? 16 : 0
              }}>
                <Text style={{
                  color: isSelected ? 'white' : (isToday ? '#4285F4' : (isDisabled ? '#d9e1e8' : '#333')),
