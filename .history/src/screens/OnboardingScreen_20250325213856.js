      ...
      if (userId) {
        try {
          ...
          if (rpcError) {
            ...
            // Instead of using admin API, try a different approach
            if (userEmail && userId) {
              console.log('Attempting to create session for:', userEmail);
              try {
                // Try to fetch the user to confirm they exist
                const { data: userData, error: userError } = await supabase.auth.getUser();
                
                if (userError) {
                  console.log('Error getting user:', userError.message);
                  
                  // Since we can't get current user, try a simpler approach:
                  // Just send a magic link to the user's email
                  ...
                  if (magicLinkError) {
                    ...
                    Alert.alert(
                      ...
                      'Wir haben dir einen Link zur Anmeldung an deine E-Mail-Adresse geschickt.',
                      [{ text: 'OK' }]
                    );
                  }
                } else if (userData) {
                  console.log('User data retrieved:', userData.user.id);
                  // We have the user, try to trigger auth state change
                  const { data: authData } = await supabase.auth.getSession();
                  
                  if (authData?.session) {
                    console.log('Session exists, user should be logged in');
                    // User is already logged in, trigger a navigation
                    return;
                  } else {
                    console.log('No session for existing user, sending sign-in email');
                    // Send sign-in email
              ...
              Alert.alert(
                ...
                'Dein Profil wurde erstellt. Du kannst dich jetzt anmelden.',
                [{ text: 'OK' }]
              );
