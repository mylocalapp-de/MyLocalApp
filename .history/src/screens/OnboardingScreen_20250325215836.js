...
import { saveLocalPreferences } from '../utils/localStorage';
...
const OnboardingScreen = ({ navigation, route }) => {
  ...
  const { setPreferences, user, isLocalAccount, localUser } = useAuth();
  
  // Extract email from route params
  const userEmail = route.params?.tempEmail;
  
  useEffect(() => {
    console.log('OnboardingScreen loaded, local account:', isLocalAccount);
    
    // We can initialize immediately for local accounts
    if (isLocalAccount || localUser) {
      setInitializing(false);
    } else {
      // Still try to get session for Supabase accounts
      checkSession();
    }
  }, [isLocalAccount, localUser]);
  ...
  const handleComplete = async () => {
    ...
    try {
      console.log('Saving preferences, local account:', isLocalAccount);
      
      // Use setPreferences from context (which now handles both local and Supabase accounts)
