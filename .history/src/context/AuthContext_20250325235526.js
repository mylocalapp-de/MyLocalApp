...
export const AuthProvider = ({ children }) => {
  ...
  const signIn = async (email, password) => {
    try {
      ...
      if (!userId) {
        console.error('No user ID returned from verify_password');
        return { 
          success: false, 
          error: {
      ...
      if (userError || !userData) {
        console.error('Failed to get user data after authentication:', userError);
        return { 
          success: false, 
          error: {
  ...
  // Create value object to expose context
  const value = {
    user,
    loading,
    preferences,
