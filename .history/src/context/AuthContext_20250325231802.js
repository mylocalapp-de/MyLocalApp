...
export const AuthProvider = ({ children }) => {
  ...
  const upgradeToFullAccount = async (email, password) => {
    try {
      ...
      try {
        // This call will only succeed for admin role, but it helps identify issues
        const { data: existingUsers, error: checkError } = await supabase.auth.admin.listUsers();
        
        if (!checkError && existingUsers) {
          const emailExists = existingUsers.users.some(user => user.email === email);
          if (emailExists) {
            console.warn('Email appears to already exist in the system:', email);
            return { 
              success: false, 
              error: { message: 'This email is already registered. Please use a different email.' }
            };
          }
        }
      } catch (adminCheckError) {
        // Ignore error - admin API may not be accessible
        console.log('Admin check not available (expected):', adminCheckError);
      }
      
      // First try to sign up the user
      console.log('Attempting to create account with Supabase auth.signUp...');
      ...
      if (error) {
        ...
        if (error.message === 'Database error saving new user') {
          console.log('Received database error - checking if user might already exist...');
          
          // Try to sign in with these credentials to see if account already exists
          ...
          if (!signInError && signInData?.user) {
            console.log('Account appears to already exist, signing in instead...');
            return { success: true, data: signInData };
          } else {
            console.log('Sign in attempt failed, original error is valid:', signInError);
          }
        }
      ...
      console.log('Account creation successful, waiting for trigger execution...');
