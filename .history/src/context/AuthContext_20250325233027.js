...
export const AuthProvider = ({ children }) => {
  ...
  const upgradeToFullAccount = async (email, password) => {
    try {
      ...
      // Check if the email already exists to provide a better error message
      ...
      if (!emailCheckError || 
         (emailCheckError && !emailCheckError.message.includes('Email not found'))) {
        console.error('Email already exists or another issue occurred:', emailCheckError);
        return { 
          success: false, 
          error: {
            message: 'Diese E-Mail-Adresse ist bereits registriert oder es gibt ein Problem mit der Anmeldung.',
            code: 'email_exists_or_error'
          }
        };
      }
      
      console.log('Email check passed, proceeding with account creation');
      
      // Try a simplified sign-up without any custom metadata first
      console.log('Attempting simplified sign-up with email:', email);
      ...
      if (preferences && preferences.length > 0) {
        console.log('Updating user metadata with preferences...');
        ...
        if (updateError) {
          ...
          console.log('Successfully updated user metadata with preferences');
        }
      }
      ...
      // Manually create profile and preferences
      let profileCreated = false;
      let preferencesCreated = false;
      
      // Create profile manually - skip trigger dependency
      try {
        console.log('Creating profile manually...');
        
        // Try to get the profiles table structure for diagnostic purposes
        try {
          const { data: profilesMetadata, error: metadataError } = await supabase
            .from('profiles')
            .select()
            .limit(1);
            
          if (metadataError) {
            console.error('Error getting profiles table metadata:', metadataError);
          } else {
            console.log('Profiles table appears accessible');
          }
        } catch (metaErr) {
          console.error('Exception checking profiles table:', metaErr);
        }
        
        // Try to insert profile directly
        console.log('Attempting direct profile insertion with user ID:', data.user.id);
        ...
        if (insertError) {
          ...
          try {
            ...
            if (rpcError) {
              ...
              console.log('Successfully created profile via RPC');
              profileCreated = true;
            }
          } catch (rpcErr) {
      ...
      if (preferences && preferences.length > 0) {
        try {
          console.log('Inserting preferences manually:', preferences);
