...
- **Organization Mode**:
  - Toggle organization mode in the Profile screen (only available for permanent accounts)
...
- **Authentication Implementation**:
  - One-click sign up: press "Neu hier?" to enter the app immediately
  - Local storage is used for initial account creation with default settings
...
2. New user selects "Neu hier?" and immediately enters the main app with:
   - Default display name "Neuer Account" 
   - All content categories (Kultur, Sport, Verkehr, Politik) pre-selected
3. In Profile tab, user can:
   - Change display name
   - Modify content preferences
   - Create a permanent account with email and password
4. Account data is securely stored in Supabase for permanent accounts
5. Organization features are only available to permanent accounts
